...
function Admin() {
  ...
  const [newStatus, setNewStatus] = useState<'live' | 'scheduled' | 'finished'>('scheduled');
  ...
  const handleCreateMatch = async (e: React.FormEvent) => {
    ...
    try {
      const created = await matchApi.createMatch(homeTeamName, awayTeamName, newStatus);
      setMessage(`✅ Match created: ${created.homeTeam} vs ${created.awayTeam}`);
      setHomeTeamName('');
      setAwayTeamName('');
  ...
  return (
    <div className="min-h-screen pb-10">
      ...
      <div className="px-5">
        <Link to="/" className="inline-block mb-5 px-6 py-3 bg-white/20 text-white rounded-lg hover:bg-white/30 hover:-translate-x-1 transition-all">
          ← Back to Matches
        </Link>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl p-8 shadow-lg">
            <h2 className="text-2xl font-bold mb-5 text-gray-800">Create Match</h2>
            <form onSubmit={handleCreateMatch} className="space-y-4">
              ...
              <select
                value={newStatus}
                onChange={(e) => setNewStatus(e.target.value as 'live' | 'scheduled' | 'finished')}
                className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-primary focus:outline-none transition"
              >
