...
function MatchDetail() {
  ...
  // Ensures SSEService is created once
  ...
  if (!match)
    return (
      <div className="flex items-center justify-center min-h-screen text-red-400 text-2xl">
        Match not found
      </div>
    );
  ...
  return (
    <div className="min-h-screen pb-10">
      ...
      <div className="px-5">
        <Link
          to="/"
          className="inline-block mb-5 px-6 py-3 bg-white/20 text-white rounded-lg hover:bg-white/30 hover:-translate-x-1 transition-all"
        >
          ← Back to Matches
        </Link>
            ...
            <div className="space-y-4">
              {[...match.goals]
                ...
                .map((goal) => (
                  <div
                    key={goal.id}
                    className={`flex items-center gap-5 p-4 rounded-xl transition-all hover:translate-x-2 ${
                      goal.team === 'home'
                        ? 'bg-blue-50 border-l-4 border-blue-500'
                        : 'bg-orange-50 border-l-4 border-orange-500'
                    }`}
                  >
                    <span className="text-lg font-bold text-primary min-w-[50px]">
                      {goal.minute}'
                    </span>
